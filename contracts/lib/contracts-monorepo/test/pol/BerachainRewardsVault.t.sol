...
import { IBerachainRewardsVault, IPOLErrors } from "src/pol/interfaces/IBerachainRewardsVault.sol";
...
import { MockDAI, MockUSDT } from "@mock/honey/MockAssets.sol";
import { PausableERC20 } from "@mock/token/PausableERC20.sol";

contract BerachainRewardsVaultTest is DistributorTest, StakingTest {
    ...
    function _notifyRewardAmount(uint256 _amount) internal override {
        ...
        vault.notifyRewardAmount(valPubkey, _amount);
    }
    ...
    function testFuzz_ChangeInFactoryOwner(address newOwner) public {
        ...
        // change owner of rewardsVaultFactory
    ...
    function test_FailIfNotOwner() public override {
        ...
        vm.expectRevert();
        vault.notifyRewardAmount(valPubkey, 255);
    ...
    function test_SetDistributor() public {
        ...
        vm.prank(governance);
        vm.expectEmit();
        emit IBerachainRewardsVault.DistributorSet(newDistributor);
    ...
    function test_RecoverERC20_FailsIfIncentiveToken() public {
        testFuzz_WhitelistIncentiveToken(address(dai));
        vm.prank(governance);
    ...
    function test_RecoverERC20() public {
        ...
        vm.prank(governance);
        vm.expectEmit();
        emit IBerachainRewardsVault.Recovered(address(dai), 1 ether);
        vault.recoverERC20(address(dai), 1 ether);
    ...
    function performDelegateStake(address _delegate, address _user, uint256 _amount) internal {
        ...
        emit IBerachainRewardsVault.DelegateStaked(_user, _delegate, _amount);
    ...
    function test_DelegateWithdraw() public {
        ...
        vm.prank(operator);
        vm.expectEmit(true, true, true, true);
        emit IBerachainRewardsVault.DelegateWithdrawn(user, operator, 100 ether);
        ...
        public
    {
        ...
        vm.prank(_delegate);
        vm.expectEmit(true, true, true, true);
        emit IBerachainRewardsVault.DelegateWithdrawn(_user, _delegate, _withdrawAmount);
    ...
    function test_SetOperator() public {
        vm.prank(user);
        vm.expectEmit(true, true, true, true);
        emit IBerachainRewardsVault.OperatorSet(user, operator);
    ...
    function test_FailNotifyRewardNoSufficientAllowance(int256 numDays) public {
        ...
        vm.prank(address(distributor));
        vault.notifyRewardAmount(valPubkey, 100 ether);
    }
    ...
    function test_GetWhitelistedTokens() public {
        ...
        tokenAddresses[2] = address(usdt);

        for (uint256 i; i < tokenAddresses.length; ++i) {
            vm.prank(governance);
            vault.whitelistIncentiveToken(tokenAddresses[i], 100 * 1e18);
        }
    ...
    function test_WhitelistIncentiveToken_FailsIfNotOwner() external {
        ...
        vault.whitelistIncentiveToken(address(dai), 1000 * 1e18);
    }
    ...
    function test_WhitelistIncentiveToken_FailsIfAlreadyWhitelisted() external {
        ...
        vault.whitelistIncentiveToken(address(dai), 10 * 1e18);
    }
    ...
    function test_WhitelistIncentiveToken_FailsIfCountEqualToMax() external {
        ...
        vm.prank(governance);
        vm.expectRevert(IPOLErrors.TokenAlreadyWhitelistedOrLimitReached.selector);
        vault.whitelistIncentiveToken(address(dai2), 100 * 1e18);
    }

    function testFuzz_WhitelistIncentiveToken(address token) public {
        uint256 count = vault.getWhitelistedTokensCount();

        // Whitelist the token
        vm.prank(governance);
        vm.expectEmit();
        emit IBerachainRewardsVault.IncentiveTokenWhitelisted(token, 100 * 1e18);
        vault.whitelistIncentiveToken(token, 100 * 1e18);
    ...
    function test_WhitelistIncentiveToken() public {
        testFuzz_WhitelistIncentiveToken(address(dai));
        testFuzz_WhitelistIncentiveToken(address(honey));
    }
    ...
    function test_WhitelistIncentiveToken_FailsIfMinIncentiveRateIsZero() public {
        ...
        vault.whitelistIncentiveToken(address(dai), 0);
    }
    ...
    function testFuzz_WhitelistIncentiveToken_FailsIfMinIncentiveRateMoreThanMax(uint256 minIncentiveRate) public {
        ...
        vault.whitelistIncentiveToken(address(dai), minIncentiveRate);
    }
    ...
    function test_RemoveIncentiveToken() public {
        test_WhitelistIncentiveToken();
        uint256 whitelistedTokens = vault.getWhitelistedTokensCount();
        vm.prank(vaultManager);
        vm.expectEmit();
        emit IBerachainRewardsVault.IncentiveTokenRemoved(address(honey));
        vault.removeIncentiveToken(address(honey));
        (uint256 minIncentiveRate, uint256 incentiveRate,) = vault.incentives(address(honey));
        assertEq(minIncentiveRate, 0);
        assertEq(incentiveRate, 0);
        assertEq(vault.getWhitelistedTokensCount(), whitelistedTokens - 1);
    }

    function testFuzz_RemoveIncentiveToken(address token) public {
        testFuzz_WhitelistIncentiveToken(token);
        removeIncentiveToken(token);
    }

    function removeIncentiveToken(address token) public {
        uint256 count = vault.getWhitelistedTokensCount();
        vm.prank(vaultManager);
        vm.expectEmit();
        emit IBerachainRewardsVault.IncentiveTokenRemoved(token);
    ...
    function testFuzz_RemoveIncentiveToken_Multiple(uint8 numTokens, uint256 seed) public {
        ...
        for (uint256 i; i < numTokens; ++i) {
            vault.whitelistIncentiveToken(address(new MockDAI()), 100 * 1e18);
        }
        vm.stopPrank();
    ...
    function test_SetMaxIncentiveTokensCount() public {
        vm.prank(governance);
        vm.expectEmit();
        emit IBerachainRewardsVault.MaxIncentiveTokensCountUpdated(2);
    ...
    // test the decrease in incentive rate when amountRemaining is not required to be 0.
    ...
    function test_ProcessIncentives() public {
        addIncentives(100 * 1e18, 200 * 1e18);
        // coinsabe emit 0.1 BGT to the vault
        vm.startPrank(address(distributor));
        IERC20(bgt).safeIncreaseAllowance(address(vault), 1 ether);
        vm.expectEmit();
        emit IBerachainRewardsVault.IncentivesProcessed(valPubkey, address(dai), 1e17, 20 * 1e18);
        emit IBerachainRewardsVault.IncentivesProcessed(valPubkey, address(honey), 1e17, 20 * 1e18);
        vault.notifyRewardAmount(valPubkey, 1e17);
        (,, uint256 amountRemainingUSDC) = vault.incentives(address(dai));
        (,, uint256 amountRemainingHoney) = vault.incentives(address(honey));
        // coinbase will get 200(incentiveRate) * 0.1 = 20 tokens of dai and honey
        assertEq(amountRemainingUSDC, 80 * 1e18);
        assertEq(amountRemainingHoney, 80 * 1e18);
        assertEq(dai.balanceOf(operator), 20 * 1e18);
        assertEq(honey.balanceOf(operator), 20 * 1e18);
        vm.stopPrank();
        // Since amountRemaining is less than minIncentiveRate, incentiveRate can be updated here.
        ...
        assertEq(amountRemaining, 180 * 1e18);
    }
    ...
    function test_ProcessIncentives_NotFailWithMaliciusIncentive() public {
        ...
        emit IBerachainRewardsVault.IncentivesProcessed(valPubkey, address(dai), 1e17, 20 * 1e18);
        emit IBerachainRewardsVault.IncentivesProcessed(valPubkey, address(honey), 1e17, 20 * 1e18);
        emit IBerachainRewardsVault.IncentivesProcessFailed(valPubkey, address(pausableERC20), 1e17, 20 * 1e18);
        vault.notifyRewardAmount(valPubkey, 1e17);
    ...
    function addIncentives(uint256 amount, uint256 _incentiveRate) internal {
        ...
        // mint honey and approve vault to use the tokens on behalf of the user
        honey.mint(address(this), type(uint256).max);
        honey.approve(address(vault), type(uint256).max);

        vm.expectEmit();
        emit IBerachainRewardsVault.IncentiveAdded(address(dai), address(this), amount, _incentiveRate);
        vault.addIncentive(address(dai), amount, _incentiveRate);
        vault.addIncentive(address(honey), amount, _incentiveRate);
    ...
    function addMaliciusIncentive(uint256 amount, uint256 _incentiveRate) internal {
        testFuzz_WhitelistIncentiveToken(address(pausableERC20));
        // mint dai and approve vault to use the tokens on behalf of the user
        pausableERC20.mint(address(this), type(uint256).max);
        pausableERC20.approve(address(vault), type(uint256).max);

        vm.expectEmit();
        emit IBerachainRewardsVault.IncentiveAdded(address(pausableERC20), address(this), amount, _incentiveRate);
