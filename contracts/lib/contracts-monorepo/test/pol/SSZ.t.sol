...
import { SSZ } from "src/pol/beacon/SSZ.sol";

contract SSZTest is Test {
    function test_uint64HashTreeRoot() public pure {
        uint64 w = type(uint64).max;
        bytes32 expected = bytes32(bytes.concat(hex"FFFFFFFFFFFFFFFF", bytes24(0)));
    ...
    function test_addressHashTreeRoot() public pure {
        ...
        bytes32 expected = bytes32(bytes.concat(hex"1234567890ABCDEF", bytes24(0)));
        bytes32 actual = SSZ.addressHashTreeRoot(a);
        assertEq(actual, expected);

        address b = address(0x0102030405060708090a0B0c0d0e0f1011121314);
        expected = hex"0102030405060708090a0b0c0d0e0f1011121314000000000000000000000000";
    ...
    function test_ValidatorPubkeyRoot() public view {
        ...
        bytes32 expected = 0x8d94e5939d9e60417b14cbe52b90124eeb99bee4a0336219fe04d693e64944d9;
