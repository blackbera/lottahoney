...
contract BeaconVerifier is Verifier, Ownable, IBeaconVerifier {
    ...
    function setExecutionNumberGIndex(uint256 _executionNumberGIndex) external onlyOwner {
        executionNumberGIndex = _executionNumberGIndex;
        emit ExecutionNumberGIndexChanged(_executionNumberGIndex);
    }

    function setExecutionFeeRecipientGIndex(uint256 _executionFeeRecipientGIndex) external onlyOwner {
        executionFeeRecipientGIndex = _executionFeeRecipientGIndex;
    ...
    /// @inheritdoc IBeaconVerifier
    function getParentBeaconBlockRootAt(uint64 timestamp) external view returns (bytes32) {
        ...
        view
    {
        proveValidatorPubkeyInBeaconBlock(
            getParentBlockRoot(timestamp), proposerPubkeyProof, proposerPubkey, proposerIndex
        );
    }
    ...
    function verifyExecutionNumber(
        ...
        view
    {
        proveExecutionNumberInBeaconBlock(getParentBlockRoot(timestamp), executionNumberProof, executionNumber);
    }

    /// @inheritdoc IBeaconVerifier
    /// @dev gas used ~41784
    function verifyCoinbase(uint64 timestamp, address coinbase, bytes32[] calldata coinbaseProof) external view {
        proveExecutionFeeRecipientInBeaconBlock(getParentBlockRoot(timestamp), coinbaseProof, coinbase);
    }
        ...
        view
    {
        ...
        if (!SSZ.verifyProof(executionNumberProof, beaconBlockRoot, executionNumberRoot, executionNumberGIndex)) {
            revert InvalidProof();
        }
    }
        ...
        view
    {
        ...
        if (!SSZ.verifyProof(coinbaseProof, beaconBlockRoot, coinbaseRoot, executionFeeRecipientGIndex)) {
            revert InvalidProof();
        }
    }
}
